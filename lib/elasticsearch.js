...
var es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData, listGaugeData) {
  ...
  var statsdIndex = elasticIndex + '-{0}-' + indexDate.getUTCFullYear()
  ...
  for (key in listCounters) {
    payload += '{"index":{"_index":"' + format(statsdIndex, elasticCountType) + '","_type":"' + elasticCountType + '"}}' + "\n";
    payload += '{';
    innerPayload = '';
  ...
  for (key in listTimers) {
    payload += '{"index":{"_index":"' + format(statsdIndex, elasticTimerType) + '","_type":"' + elasticTimerType + '"}}' + "\n";
    payload += '{';
    innerPayload = '';
  ...
  for (key in listTimerData) {
    payload += '{"index":{"_index":"' + format(statsdIndex, elasticTimerDataType) + '","_type":"' + elasticTimerDataType + '"}}' + "\n";
    payload += '{';
    innerPayload = '';
  ...
  for (key in listGaugeData) {
    payload += '{"index":{"_index":"' + format(statsdIndex, elasticGaugeDataType) + '","_type":"' + elasticGaugeDataType + '"}}' + "\n";
    payload += '{';
    innerPayload = '';
  ...
  var req = request(optionsPost, function (res) {
...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  request = require(configEs.transport || 'http').request
